		...
		if (choice.length() != 1) {
			...
			case 'L': case 'l': {
				...
				try {
					...
					ifs >> game;
					ifs.close();
				} catch (const Chess::Exception& game_err) {
					std::cout << game_err.what() << std::endl;
					return -1;
				}
			...
			case 'M': case 'm': {
				...
				// And carry out the move (catching exceptions)
				else {					
					try {
							game.make_move(std::make_pair(argument[0], argument[1]),
										   std::make_pair(argument[2], argument[3]));
					} catch (const Chess::Exception& mv_err) {
						std::cerr << "Could not make move: " << mv_err.what() << std::endl;
					}
				}
				break;
			}
